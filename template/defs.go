...
func (m *{{.DefaultModelName}}) InsertWithTx(ctx context.Context, tx *gorm.DB, data *{{.BizStructName}}) (int64, error) {
	entityData := new({{.EntityStructName}})
	entityData.FromBizStruct(data)
    err := tx.WithContext(ctx).Model(&{{.EntityStructName}}{}).Create(entityData).Error
    if err != nil {
        return 0, err
    }
...
func (m *{{.DefaultModelName}}) UpdateWithTx(ctx context.Context, tx *gorm.DB, data *{{.BizStructName}}, columns ...string) error {
	...
	var err error
	if len(columns) > 0 {
		err = tx.WithContext(ctx).Model(&{{.EntityStructName}}{}).Select(columns).Where("id = ?", entityData.ID).Updates(entityData).Error
	} else{
		err = tx.WithContext(ctx).Model(&{{.EntityStructName}}{}).Where("id = ?", entityData.ID).Updates(entityData).Error
	}
